# -*- coding: utf-8 -*-
"""
Created on Mon Jun 10 19:08:47 2019

@author: Thyago

Agrupamento de SDA com algoritmo Kmeans e Método Elbow do Pyclustering
"""
...
def calculate_k_from_silhouette_coefficient(data, kmin, kmax):
    ...
    for k in range(kmin, kmax + 1):
        clusters, final_medoids = clustering_with_kmedoids(data, k)
        score_silhouette = silhouette(data, clusters).process().get_score()
        mean_silhouette.append(sum(score_silhouette) / len(score_silhouette))
        k_clusters.append(k)
    max_silhouette = max(mean_silhouette)
    index_max_silhouette = mean_silhouette.index(max_silhouette)
...
def avaliate_clustering_with_silhouette_coefficient(data, clusters):
    score_silhouette = silhouette(data, clusters).process().get_score()
    group_silhouette = sum(score_silhouette) / len(score_silhouette)
    return group_silhouette



def clustering_with_kmedoids(data, k_clusters, initial_medoids=0):
    # Set random initial medoids.
    if initial_medoids == 0:
        initial_medoids =  [np.random.randint(len(data)) for x in range(k_clusters)]
    else:
    ...
    return clusters, final_medoids



def visualize_clustering(axix, axiy, optimal_clusters_kmedoids, optimal_final_medoids):
    # Visualize obtained results    
    # kmeans_visualizer.show_clusters(coordenadas, clusters, final_centers)
#    visualizer = cluster_visualizer_multidim()
#    visualizer.append_clusters(clusters, listAxis)
#    visualizer.show()
    colors_medoids = np.zeros(len(axix))
    c = 0
    for j in optimal_clusters_kmedoids:
        for i in j:
            colors_medoids[i] = c
        c += 1
    name_fig = 'imgs/rede_agrupada_kmedoids_k_' + str(len(optimal_clusters_kmedoids)) + '.png'
    plt.figure(figsize=(9, 9))
    plt.title('Agrupamento da rede com K-Medoids e k=' + str(len(optimal_clusters_kmedoids)))
    plt.xlabel('Coordenada X')
    plt.ylabel('Coordenada Y')
    plt.scatter(axix, axiy, c=colors_medoids)
    plt.savefig(name_fig)
    plt.show()
    
    
    for medoid in optimal_final_medoids:
        colors_medoids[medoid] = c
    # name_fig = 'imgs/medoids_kmedoids_k_' + str(len(optimal_clusters_kmedoids)) + '.eps'
    plt.figure(figsize=(9, 9))
    plt.title('Medoids dos Clusters')
    plt.scatter(axix, axiy, c=colors_medoids)
    # plt.savefig(name_fig)
    plt.show()
...
# Calcula número ótimo de k-clusters
kmin = 2
kmax = 100

# Calcula número ótimo de k-clusters com o Coeficiente de Silhueta
...
#print('Coeficiente de Silhueta máximo obitido para', k_clusters_silhouette, 'clusters: ', max_silhouette)

# Clustering K-Means with optimal_k_clusters
group_silhouette = []
optimal_clusters_kmedoids = []
optimal_final_medoids = []
for i in range(kmax):
    clusters_kmedoids, final_medoids = clustering_with_kmedoids(junctions_array_normalized, k_clusters_silhouette)
    group_silhouette.append(avaliate_clustering_with_silhouette_coefficient(junctions_array_normalized, clusters_kmedoids))
    optimal_clusters_kmedoids.append(clusters_kmedoids)
    optimal_final_medoids.append(final_medoids)

best_group_silhouette = max(group_silhouette)
optimal_clusters_kmedoids = optimal_clusters_kmedoids[group_silhouette.index(best_group_silhouette)]
optimal_final_medoids = optimal_final_medoids[group_silhouette.index(best_group_silhouette)]


# create array with node names and node clusters
junctions_clusters = np.arange(len(normal_coordinatesx))
num_of_cluster = 0
for i in optimal_clusters_kmedoids:
    for j in i:
...
array_junctions_clusters = np.array([list(junctions_names), 
 ...
 # Visualize obtained results 
print('\n\nK-medoids com k ótimo de %d clusters' %(k_clusters_silhouette)) 
visualize_clustering(dataFrame_junctions['Coordinatex'], dataFrame_junctions['Coordinatey'], optimal_clusters_kmedoids, optimal_final_medoids)

print('\nCoeficiente de silhueta do grupo: ', best_group_silhouette)
np.save('saves/silhouette_exnet_kmedoids.npy', best_group_silhouette)
print('\nMedoids Finais ótimos: ') #indices dos medoids

j = 1
for i in optimal_final_medoids:
    print('Medoid', j, ':', junctions_array_normalized[i, :2])
    j += 1
...
pd.DataFrame({'Node' : nodes_tocsv, 'Cluster' : clusters_tocsv}).to_csv('saves/setores_exnet_kmedoid.csv', index=False)
np.save('saves/nodes_kmedoids.npy', nodes_tocsv)
np.save('saves/clusters_kmedoids.npy', clusters_tocsv)
...
np.save('saves/frontier_links_exnet_kmedoids.npy',frontier_links)
np.save('saves/prvs_cost_exnet_kmedoids.npy',cost_prv)
